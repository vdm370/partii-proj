...
We can find a bijection between each insertion step in the creation of the tour using this algorithm and one of the MST's edges, such that the condition that the insert cost is not more than twice expensive than the MST edge. After summing all that up, we can use \hyperref{mstlemma}[Lemma 1] to show that the theorem holds. More details can be found here\cite{Rosenkrantz_1977}.
